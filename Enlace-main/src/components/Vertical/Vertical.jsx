              ...
              Object.entries(events).map((event, index) => {
                return (
                  event[1].map((e, i) => {
                    return (
                      <>
                        {i === 0 ? <p className="text-white font-semibold font-reemkufi text-4xl py-7">Day {index + 1}</p> : ""}
                        {i === 0 ? <p className="text-white font-semibold font-reemkufi text-3xl pb-3" style = {{marginTop: "-1rem"}}>{startDate + index}{postfix(startDate + index)} March</p> : ""}  
